...
app.post("/api/webhook/hashpay", (req, res) => {
  ...
  console.log(`Payment ${isSuccessful ? 'successful' : 'failed'} for checkout: ${CheckoutRequestID}`);
  ...
  if (isSuccessful && CheckoutRequestID) {
    ...
    if (session) {
      ...
      console.log(`💰 Amount: ${TransactionAmount}, Transaction ID: ${TransactionID}`);
      ...
      if (
        ...
        console.log(`📡 WebSocket notification sent for checkout: ${CheckoutRequestID}`);
      } else {
        console.log(`⚠️ No active WebSocket connection for checkout: ${CheckoutRequestID}`);
      }
    ...
    if (session) {
      ...
      console.log(`❌ Payment failed for checkout: ${CheckoutRequestID}, Code: ${ResponseCode}`);
    }
  }
...
app.post("/api/initiate-payment", async (req, res) => {
  ...
  try {
    ...
    if (data.ResponseCode === "0" && data.CheckoutRequestID) {
      ...
      console.log(`✅ Payment initiated successfully. Checkout ID: ${data.CheckoutRequestID}`);
      ...
      console.log(`❌ Payment initiation failed: ${data.ResponseDescription || 'Unknown error'}`);
      res.status(400).json({
        success: false,
      ...
      if (data.type === "register" && data.checkoutId) {
        ...
        if (session) {
          ...
          if (session.status === "completed") {
            ...
            console.log(`📡 Sent cached payment completion for: ${data.checkoutId}`);
          }
        } else {
...
server.listen(PORT, () => {
  ...
  console.log(`💰 Payment initiation: POST http://localhost:${PORT}/api/initiate-payment`);
  console.log(`✅ Payment status: POST http://localhost:${PORT}/api/check-payment-status`);
  console.log(`🔄 Webhook endpoint: POST http://localhost:${PORT}/api/webhook/hashpay`);
